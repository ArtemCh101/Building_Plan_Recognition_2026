...
prompts = {
    "Simple": {
        "text": ". ".join(classes) + ".",
        "mappings": {c: c.lower() for c in classes}
    },
    "Descriptive": {
        "text": "thick solid structural walls. thin parallel window lines. swinging doors with arcs. indoor room living spaces. staircase rectangular steps. kitchen counters and sinks. bathroom toilets. storage closet spaces. outdoor balcony platforms. elevator lift shafts.",
        "mappings": {
            "Wall": "wall",
            "Window": "window",
            "Door": "door",
...
for p_name, p_config in prompts.items():
    ...
    for img_info in test_data:
        ...
        for target_cls in classes:
            mapping_keyword = mappings[target_cls]
            indices = [i for i, label in enumerate(results["labels"]) if mapping_keyword in label.lower()]
