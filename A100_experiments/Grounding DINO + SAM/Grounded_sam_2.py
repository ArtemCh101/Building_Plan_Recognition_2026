...
prompts = {
    "Simple": ". ".join(classes) + ".",
    "Descriptive": "2D architectural building plan containing: " + ", ".join(classes) + "."
}
...
for p_name, p_text in prompts.items():
    ...
    for img_info in test_data:
        ...
        for target_cls in classes:
            indices = [i for i, label in enumerate(results["labels"]) if target_cls.lower() in label.lower()]
            if not indices:
            ...
            if target_cls in gt_data:
                ...
                sam_in = sam2_processor(image_pil, input_boxes=[final_boxes.cpu().tolist()], return_tensors="pt").to(device)
                with torch.no_grad():
